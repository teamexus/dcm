...
def create_appointment(request):
    ...
    if request.method == 'POST':
        ...
        try:
            Appointment.objects.create(user=user, doctor=doctor,  patient=patient, appointment=p, mobile=m, date1=d1, time1=t1)
            error="no"
        except:
            error="yes"
...
def delete_appointment(request, pid):
    ...
    appointment.delete()
    return redirect('/diagnostic_center/view_appointment')
